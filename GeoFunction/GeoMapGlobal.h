...
class CLZGUI_EXPORT ImPlotPointArray{
public:
...
struct TilePos {
    ...
    GeoRect toGeoRect() const {
        const auto lef_top = [](const TilePos& tilePos) -> GeoPos {
            const int zoom = tilePos.z;
        ...
        const GeoPos pos1 = lef_top(*this);
        const GeoPos pos2 = lef_top(TilePos(x + 1, y + 1, z));
