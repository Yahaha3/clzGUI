...
ImGuiContext *QVulkanRenderer::ctx()
{
    return ctx_;
}
...
QVulkanRenderer *VulkanWindow::render()
{
    return dynamic_cast<QVulkanRenderer*>(render_);
}
...
//    qDebug() << "[Event Type]: " << type;
    if(type == QEvent::Expose && rd && rd->inited_){
        // nothing todo
    } else {
        res = QVulkanWindow::event(e);
    }
...
{
    if(!m_global_timer){
        m_global_timer = std::make_shared<QTimer>();
        connect(m_global_timer.get(), &QTimer::timeout, this, &VulkanWindow::slot_global_timeout);
        m_global_timer->start(10); // 期望触发频率高一些吧
    }
}

void VulkanWindow::update_render_ctx()
{
    auto rd = dynamic_cast<QVulkanRenderer*>(render_);
//    rd->ctx = ImGui::CreateContext();
    ImGui::SetCurrentContext(rd->ctx_);
}

bool VulkanWindow::can_render()
{
    // 如果当前鼠标是按下状态直接返回true
    auto rd = render();
    if(rd && rd->g_mouse_pressed[0]){
    ...
    auto delta = QDateTime::currentMSecsSinceEpoch() - m_update_time;
    auto render_interval = (1000.0f / m_render_fps);
...
void VulkanWindow::slot_global_timeout()
{
    if(can_render()){
        frameReady();
        requestUpdate();
