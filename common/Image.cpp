...
void ImageVk::set_image_path(const QString &path)
{
    m_path = path;
}

QString ImageVk::get_image_path() const
{
    return m_path;
}

uchar *ImageVk::get_image_data(const QByteArray &byte, int &w, int &h)
{
    char* data = byte.toStdString().data();
    size_t size = byte.toStdString().length();
    int n = 0;
    unsigned char *image_data = stbi_load_from_memory(reinterpret_cast<unsigned char*>(data), size,
                                                      &w, &h, &n, 4);
    return image_data;
