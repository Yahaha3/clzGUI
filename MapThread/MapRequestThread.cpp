...
clz::MapRequestThread::MapRequestThread(GeoMap* map, QObject *parent) : QThread(parent)
{
    ...
    m_cdboc = std::make_shared<clz::ClzDataBaseOnlineCache>();
    m_cdboc->init();
    m_cdboc->update_tile_server_address(m_map->geo_backend()->server(), m_brand, m_style);
...
void clz::MapRequestThread::run()
{
    while (!m_stop) {
        ...
        lock.unlock();
        {
            ...
            if(!ok) {
                result.result.insert(RequestDBResultEnum::RequestError);
                m_cdboc->request_tile(one.x, one.y, one.z);
//                m_cdbm->nofly_query();
            }
