...
#include "GeoFunction/LayerBackend/GeoBackendGoogle.h"
#include "Projection/ProjectionESG3857.h"
#include "Projection/ProjectionGCJ02.h"
...
GeoMap::GeoMap()
{
    m_backend = std::make_shared<GeoBackendGoogle>();
    ...
    get_tile_manager()->set_tile_projection(new ProjectionESG3857());
    set_map_projection(new ProjectionGCJ02());
    init_map_items();
...
//    double min_y = std::clamp(view.Y.Min, 0.0, 1.0);
    ...
    m_region.clear();
    append_region(m_zoom, view.Min(), view.Max());
...
QPointF GeoMap::test_proj(ImPlotPoint pt)
{
...
std::tuple<ImPlotPoint, ImPlotPoint> GeoMap::georect2implot(GeoRect rect)
{
    auto min = geopos2implot(rect.bottomRight());
    auto max = geopos2implot(rect.topLeft());
...
// 检索当前视口范围内需要显示的瓦片
bool GeoMap::append_region(int z, ImPlotPoint min, ImPlotPoint max)
{
    auto tl = get_tile_manager()->implot2geopos(min);
    auto br = get_tile_manager()->implot2geopos(max);
    // 换算出当前瓦片计数的起始点和结束点
    auto limit = pow(2,z);
    bool covered = true, ok = false;
    QList<TilePos> tmp_list, use_tile;
    {
        auto tl_tile = get_tile_manager()->get_tile_projection()->geoToTilePos(tl, z);
        auto br_tile = get_tile_manager()->get_tile_projection()->geoToTilePos(br, z);
        // 上下左右多冗余一点
        for(int x = tl_tile.x - 1, sx = br_tile.x + 1; x <= sx; x++){
            if(x < 0 || x >= limit) continue;
            for(int y = tl_tile.y - 1, sy = br_tile.y + 1; y <= sy; y++){
...
//        auto [bmin, bmax] = position.bounds();
        auto georect = position.toGeoRect();
        auto [min, max] = get_tile_manager()->georect2implot(georect);
        if(tile){
            ...
            if(ok){
                ImPlot::PlotImage("##Tiles", ID, min, max, {0,0}, {1,1}, col);
                if (debug)
                    ImPlot::PlotText(position.tozxy().toStdString().c_str(),(min.x+max.x)/2,(min.y+max.y)/2);
            }
        }
    }
}
