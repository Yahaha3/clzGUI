...
void clz::TileManager::set_tile_projection(Projection *projection)
{
    m_tile_projection.reset(projection);
}

std::shared_ptr<Projection> clz::TileManager::get_tile_projection() const
{
    return m_tile_projection;
}

clz::GeoPos clz::TileManager::implot2geopos(ImPlotPoint pt)
{
    if(!get_tile_projection()) return clz::GeoPos();
    auto scene_pos = clz::GeoFunctionUtility::implot2scene(pt);
    auto proj_pos = get_tile_projection()->sceneToMap(QPointF(scene_pos.x, scene_pos.y));
    auto geo_pos = get_tile_projection()->projToGeo(proj_pos);
    return geo_pos;
}

ImPlotPoint clz::TileManager::geopos2implot(clz::GeoPos pt)
{
    if(!get_tile_projection()) return {};
    auto proj_pos = get_tile_projection()->geoToProj(pt);
    auto scene_pos = get_tile_projection()->mapToScene(proj_pos);
    auto plot_pos = clz::GeoFunctionUtility::scene2implot(ImPlotPoint(scene_pos.x(), scene_pos.y()));
    return plot_pos;
}

std::tuple<ImPlotPoint, ImPlotPoint> clz::TileManager::georect2implot(clz::GeoRect rect)
{
    if(!get_tile_projection()) return {};
